 ...
 * @圈子播放控制
 */
public class PlayerMusicControlView extends PlayerGestureControlView
    ...
    public PlayerMusicControlView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public PlayerMusicControlView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void start() {
        switch (mIMediaPlayer == null ? -2 : mIMediaPlayer.getCurrentState()) {
            ...
            case IMediaPlayer.STATE_ERROR://播放错误
                ...
                MusicPlayerProvide.getInstance().createPlayer();
                setIMediaPlayer(MusicPlayerProvide.getInstance().getMediaPlayer());
                MusicPlayerProvide.getInstance().prepareStart(mPlayerBean.getVideo().getLink(), null, this);
