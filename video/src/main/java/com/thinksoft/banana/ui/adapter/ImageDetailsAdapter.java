 ...
 * @
 */
public class ImageDetailsAdapter extends BaseCompleteRecyclerAdapter<CircleItem> {
    public ImageDetailsAdapter(Context context) {
        super(context);
    }

    public ImageDetailsAdapter(Context context, OnAppListener.OnAdapterListener listener) {
    ...
    protected void setItemLayout() {
        super.setItemLayout();
        addItemLayout(Constant.TYPE_ITEM_1, R.layout.item_imge_details_content);
    ...
    private void binditem1(BaseViewHoder holder, int position, final CircleItem item) {
        ImagInfoDataBean bean = (ImagInfoDataBean) item.getData();
