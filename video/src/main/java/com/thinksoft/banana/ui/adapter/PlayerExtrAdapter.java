 ...
 * @
 */
public class PlayerExtrAdapter extends BaseCompleteRecyclerAdapter<PlayerItem> {
    public PlayerExtrAdapter(Context context) {
        super(context);
    }

    public PlayerExtrAdapter(Context context, OnAppListener.OnAdapterListener listener) {
    ...
    private BaseCompleteRecyclerAdapter<PerformerDataBean> mPerformerAdapter = new BaseCompleteRecyclerAdapter<PerformerDataBean>(getContext()) {
        ...
        protected void onBindBaseViewHoder(BaseViewHoder holder, int position, final PerformerDataBean item) {
            ...
            holder.getViewById(R.id.contentRoot).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    PlayerExtrAdapter.this.getListener().onInteractionAdapter(Constant.TYPE_ITEM_7, BundleUtils.putSerializable(item));
                }
            });
        }
    };
