...
import com.thinksoft.banana.ui.view.titlebar.PlayerTitleBar;
...
public class PlayerActivity
    ...
    PlayerTitleBar mPlayerTitleBar;
    ...
    public void onInteractionAdapter(int action, Bundle bundle) {
        ...
        switch (action) {
            case Constant.TYPE_ITEM_1:
                ...
                maps.put("token", UserInfoManage.getInstance().getUserInfo().getToken());
                getPresenter().getData(ApiRequestTask.home.TAG_FABULOUS, maps);
                break;
...
//              mPlayerAdapter.notifyDataSetChanged();
    ...
    public void onInteractionView(int action, Bundle bundle) {
        ...
        switch (action) {
            ...
            case Constant.ACTION_PLAYER_LANDSCAPE:
                ...
                switch (mPlayerTitleBar.mPlayerView.getOrientation()) {
                    case 90:
    ...
    protected ITitleBar buildTitleBar() {
        return mPlayerTitleBar = new PlayerTitleBar(this);
    }
    ...
    public void onBackPressed() {
        if (mPlayerTitleBar == null || mPlayerTitleBar.mPlayerView == null) {
            super.onBackPressed();
            return;
        }
        if (!mPlayerTitleBar.mPlayerView.isHorizontalScreen())
            super.onBackPressed();
        else
            onInteractionView(Constant.ACTION_PLAYER_PORTRAIT, null);
    }
        ...
        switch (sign) {
            case -1:
                ...
                if (playerBean.getVideo().getIs_free() == 0) {
                    mPlayerTitleBar.mPlayerControlView.paySuccess();
                }
                break;
            case ApiRequestTask.home.TAG_VIDEO_INFO:
                ...
                PlayerProvide.getInstance().release();
                mPlayerTitleBar.setData(mPlayerBean);
                //请求演员列表
                HashMap<String, Object> maps = new HashMap<>();
                maps.put("id", mVideoId);
                maps.put("token", UserInfoManage.getInstance().getUserInfo().getToken());
            ...
            case ApiRequestTask.home.TAG_ADD_COLLECTION_FILM:
                ...
                mPlayerTitleBar.setCollection(mPlayerBean.getCollcetion() == 1);
