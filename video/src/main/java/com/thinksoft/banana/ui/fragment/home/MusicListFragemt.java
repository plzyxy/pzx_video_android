...
import com.thinksoft.banana.entity.bean.music.MusicListBean;
...
import com.thinksoft.banana.ui.activity.home.MusicDetailsActivity;
import com.thinksoft.banana.ui.activity.my.SpreadLinkActivity;
import com.thinksoft.banana.ui.adapter.MusicListAdapter;
...
public class MusicListFragemt
    ...
    ADBean mADBean;
    MusicDataBean mMusicDataBean;

    @Override
    protected BaseRecyclerAdapter buildAdapter() {
        return new MusicListAdapter(getContext(), this);
    }
    ...
    private void requestList(int pageIndex) {
        ...
        getPresenter().getData(ApiRequestTask.home.TAG_MUSIC_LIST, maps);
    }


    private void requestBanner() {
        getPresenter().getData(ApiRequestTask.home.TAG_MUSIC_BANNER);
    }

    private void requestAD() {
        getPresenter().getData(ApiRequestTask.home.TAG_MUSIC_ADVERT);
    }

    private List<CircleItem> newItem(MusicListBean bean) {
        ...
        //内容条目
        if (bean != null && bean.getMusicList() != null && bean.getMusicList().size() > 0) {
            if (mADBean == null || mADBean.getBanners() == null || mADBean.getBanners().size() == 0) {

                for (int i = 0; i < bean.getMusicList().size(); i++) {
                    MusicDataBean circleBean = bean.getMusicList().get(i);
                ...
                for (int i = count; i < count + bean.getMusicList().size(); i++) {

                    if (i != 0 && i % 5 == 0) {
                        int index = i / 5 % bannersBeans.size();
                        items.add(new CircleItem(bannersBeans.get(index), Constant.TYPE_ITEM_4));
                    }
                    MusicDataBean circleBean = bean.getMusicList().get(i - count);
                    items.add(new CircleItem(circleBean, Constant.TYPE_ITEM_2));
                }
            }
        }
    ...
    public void onInteractionAdapter(int action, Bundle bundle) {
        ...
        switch (action) {
            case Constant.ACTION_CIRCLE_DETAILS:
                ...
                MusicDataBean bean = (MusicDataBean) item.getData();
                //详情
                startActivity(MusicDetailsActivity.getIntent(getContext(), bean));
                break;
            ...
            case Constant.ACTION_CIRCLE_Z:
                item = BundleUtils.getBaseItem(bundle);
                mMusicDataBean = (MusicDataBean) item.getData();
                //点赞
                HashMap<String, Object> maps = new HashMap<>();
                maps.put("id", mMusicDataBean.getId());
    ...
    public void httpOnSuccess(int sign, JsonElement data, String message) {
        switch (sign) {
            case ApiRequestTask.home.TAG_MUSIC_BANNER:
                //Banner 图
                mBannerBean = JsonTools.fromJson(data, BannerBean.class);
                requestAD();
                break;
            case ApiRequestTask.home.TAG_MUSIC_ADVERT:
                //列表穿插广告
            ...
            case ApiRequestTask.home.TAG_MUSIC_LIST:
                //列表
                MusicListBean bean = JsonTools.fromJson(data, MusicListBean.class);
                refreshData(newItem(bean));
                break;

            case ApiRequestTask.home.TAG_MUSIC_LOVE:
                //点赞
                if (mMusicDataBean.getIslove() == 1) {
                    mMusicDataBean.setIslove(0);
                    mMusicDataBean.setLove(mMusicDataBean.getLove() - 1);
                } else {
                    mMusicDataBean.setIslove(1);
                    mMusicDataBean.setLove(mMusicDataBean.getLove() + 1);
                }
